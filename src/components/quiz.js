// ===================================================
// PATH FORGE - QUIZ UI COMPONENT
// Question walkthrough modal, option selection & result display
// ===================================================
...
/**
 * Initialize quiz modal controls and triggers.
 *
 * @param {object} options
 * @param {Record<string, any>} options.pathwaysData
 * @param {(role: object, categoryKey: string) => void} options.onLaunchRole
 */
export function initQuiz({ pathwaysData, onLaunchRole }) {
  ...
  const topBannerQuizBtn = $("#top-banner-quiz-btn");
...
/**
 * Opens the quiz modal and resets questions to step 0.
 */
...
/**
 * Renders the active question step or triggers results display.
 */
...
/**
 * Calculates results via QuizEngine and presents recommendation UI.
 */
function renderQuizResult() {
  const modalContent = $("#modal-content");
  if (!modalContent) return;

  const { bestCategory, matchedRole, topMatches } = calculateQuizResults(quizAnswers, pathwaysRef);
  ...
  if (!matchedRole) {
    ...
    if (retryBtn) retryBtn.addEventListener("click", openQuizModal);
    return;
  }
  ...
  const launchBtn = $("#launch-matched-roadmap");
  if (launchBtn) {
